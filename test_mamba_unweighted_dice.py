# -*- coding: utf-8 -*-
"""
Evaluate a trained Mamba3DUNet checkpoint on the BHSD **test** split.

Uses the same pipeline as mamba3d_unet_template.py end-of-training test:
...
TEMPLATE_FILE = "mamba3d_unet_template.py"
DEFAULT_OUTPUT = HERE / "mamba_test_unweighted_dice_results.txt"


def _load_template_module() -> Any:
    path = HERE / TEMPLATE_FILE
    spec = importlib.util.spec_from_file_location("_mamba_eval", path)
...
def run_eval(
    ...
    if load_info["missing_keys"] or load_info["unexpected_keys"]:
        print(
            ...
            "If scores look wrong, check BASE_CHANNELS / Mamba layout vs training Colab."
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Mamba TEST eval with unweighted Dice")
